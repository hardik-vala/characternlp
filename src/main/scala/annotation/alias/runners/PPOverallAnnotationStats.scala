 ...
 */
private object PPOverallAnnotationStats extends App {

  System.out.println("PRIDE AND PREJUDICE OVERALL ANNOTATION STATS")

  val dirpath = "data/annotations/alias-ident/pride-and-prejudice"
  val sharedDirpath = dirpath + File.separator + "shared"

  System.out.println("\n(Directory: " + sharedDirpath + ")")

  val round1Dirname = "round-1"
  val round2Dirname = "round-2"
  val round3Dirname = "round-3"

  val christyDirname = "christy"
  val estherDirname = "esther"
  val truthDirname = "truth"

  val christyAnnotationFiles: Array[File] = recursiveListFiles(new File(dirpath + File.separator + christyDirname),
    noHiddenFilenameFilter)

  val estherAnnotationFiles: Array[File] = recursiveListFiles(new File(dirpath + File.separator + estherDirname),
    noHiddenFilenameFilter)

  val christySharedAnnotationFiles: List[File] = List(round1Dirname, round2Dirname, round3Dirname).map(dn =>
    recursiveListFiles(new File(sharedDirpath + File.separator + dn + File.separator + christyDirname),
      noHiddenFilenameFilter)).flatten

  val estherSharedAnnotationFiles: List[File] = List(round1Dirname, round2Dirname, round3Dirname).map(dn =>
    recursiveListFiles(new File(sharedDirpath + File.separator + dn + File.separator + estherDirname),
      noHiddenFilenameFilter)).flatten

  val truthAnnotationFiles: List[File] = List(round1Dirname, round2Dirname, round3Dirname).map(dn =>
    recursiveListFiles(new File(sharedDirpath + File.separator + dn + File.separator + truthDirname),
      noHiddenFilenameFilter)).flatten

  System.out.println("\n# Passages Completed")
  System.out.println("Christy: %d".format(christyAnnotationFiles.size))
  System.out.println("Esther: %d".format(estherAnnotationFiles.size))

  System.out.println("\nAvg. # Tags / Passage")
  System.out.println("Christy (Overall): %.4f".format(Metrics.average(christyAnnotationFiles.map(f =>
    new AliasAnnotations(f.getPath).size))))
  System.out.println("Esther (Overall): %.4f".format(Metrics.average(estherAnnotationFiles.map(f =>
      new AliasAnnotations(f.getPath).size))))
  System.out.println("Christy (Shared): %.4f".format(Metrics.average(christySharedAnnotationFiles.map(f =>
      new AliasAnnotations(f.getPath).size))))
  System.out.println("Esther (Shared): %.4f".format(Metrics.average(estherSharedAnnotationFiles.map(f =>
      new AliasAnnotations(f.getPath).size))))
  System.out.println("Truth: %.4f".format(Metrics.average(truthAnnotationFiles.map(f =>
    new AliasAnnotations(f.getPath).size))))
  ...
  // passages).
  truthAnnotationFiles.foreach((annotationFile: File) => {
    val trueAnnotations = new AliasAnnotations(annotationFile.getPath)

    val christyAnnotations = new AliasAnnotations(annotationFile.getPath.replace(truthDirname, christyDirname))
    val estherAnnotations = new AliasAnnotations(annotationFile.getPath.replace(truthDirname, estherDirname))
